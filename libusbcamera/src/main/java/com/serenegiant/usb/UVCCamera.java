...
public class UVCCamera {
	...
	protected List<USBSize> mCurrentSizeList;
...
//    		mNativePtr = 0;	// nativeDestroyを呼ぶのでここでクリアしちゃダメ
		...
		if (DEBUG) JadeLog.v(TAG, "close:finished");
	}
	...
	public USBSize getPreviewSize() {
		USBSize result = null;
		final List<USBSize> list = getSupportedSizeList();
		for (final USBSize sz: list) {
	...
	public List<USBSize> getSupportedSizeList() {
		final int type = (mCurrentFrameFormat > 0) ? 6 : 4;
		return getSupportedSize(type, mSupportedSize);
	}

	public static List<USBSize> getSupportedSize(final int type, final String supportedSize) {
		final List<USBSize> result = new ArrayList<USBSize>();
	...
	private static final void addSize(final JSONObject format, final int formatType, final int frameType, final List<USBSize> size_list) throws JSONException {
		...
		for (int j = 0; j < size_nums; j++) {
			...
			try {
				size_list.add(new USBSize(formatType, frameType, j, Integer.parseInt(sz[0]), Integer.parseInt(sz[1])));
			} catch (final Exception e) {
				break;
			}
		}
	...
	public synchronized void setAutoFocus(final boolean autoFocus) {
		if (mNativePtr != 0) {
			int result = nativeSetAutoFocus(mNativePtr, autoFocus);
			JadeLog.e(this,"set Auto Focus"+String.valueOf(result));
		}
	}
...
//================================================================================
	...
	public synchronized void setExposureMode(final int mode){
		if (mNativePtr != 0) {
			int result = nativeSetExposureMode(mNativePtr,1);
			if (DEBUG){
				if (result == 0){
					JadeLog.e(this,"setExposureMode Success");
				}
				else{
					JadeLog.e(this,"setExposureMode Failed");
				}
			}
	...
	public synchronized void setExposureValue(int value){
		int result = nativeSetExposure(mNativePtr,value);
		if (DEBUG){
			if (result == 0){
				JadeLog.e(this,"setExposureValue Success");
			}
			else{
				JadeLog.e(this,"setExposureValue Failed");
			}
		}
	}
...
// 			   Log.d(TAG, "setZoom:zoom=" + zoom + " ,value=" + z);
		...
		if (mNativePtr != 0) {
			if ((mControlSupports == 0) || (mProcSupports == 0)) {
				...
				if (DEBUG) {
					...
					JadeLog.v(TAG, String.format("Brightness:min=%d,max=%d,def=%d", mBrightnessMin, mBrightnessMax, mBrightnessDef));
					JadeLog.v(TAG, String.format("Contrast:min=%d,max=%d,def=%d", mContrastMin, mContrastMax, mContrastDef));
					JadeLog.v(TAG, String.format("Sharpness:min=%d,max=%d,def=%d", mSharpnessMin, mSharpnessMax, mSharpnessDef));
					JadeLog.v(TAG, String.format("Gain:min=%d,max=%d,def=%d", mGainMin, mGainMax, mGainDef));
					JadeLog.v(TAG, String.format("Gamma:min=%d,max=%d,def=%d", mGammaMin, mGammaMax, mGammaDef));
					JadeLog.v(TAG, String.format("Saturation:min=%d,max=%d,def=%d", mSaturationMin, mSaturationMax, mSaturationDef));
					JadeLog.v(TAG, String.format("Hue:min=%d,max=%d,def=%d", mHueMin, mHueMax, mHueDef));
					JadeLog.v(TAG, String.format("Zoom:min=%d,max=%d,def=%d", mZoomMin, mZoomMax, mZoomDef));
					JadeLog.v(TAG, String.format("WhiteBlance:min=%d,max=%d,def=%d", mWhiteBlanceMin, mWhiteBlanceMax, mWhiteBlanceDef));
					JadeLog.v(TAG, String.format("Focus:min=%d,max=%d,def=%d", mFocusMin, mFocusMax, mFocusDef));
				}
			}
		} else {
			mControlSupports = mProcSupports = 0;
	...
	private static final void dumpControls(final long controlSupports) {
		JadeLog.i(TAG, String.format("controlSupports=%x", controlSupports));
		for (int i = 0; i < SUPPORTS_CTRL.length; i++) {
			JadeLog.i(TAG, SUPPORTS_CTRL[i] + ((controlSupports & (0x1 << i)) != 0 ? "=enabled" : "=disabled"));
		}
	}

	private static final void dumpProc(final long procSupports) {
		JadeLog.i(TAG, String.format("procSupports=%x", procSupports));
		for (int i = 0; i < SUPPORTS_PROC.length; i++) {
			JadeLog.i(TAG, SUPPORTS_PROC[i] + ((procSupports & (0x1 << i)) != 0 ? "=enabled" : "=disabled"));
		}
	}
	...
	private final String getUSBFSName(final UsbControlBlock ctrlBlock) {
		...
		if (TextUtils.isEmpty(result)) {
			JadeLog.e(TAG, "failed to get USBFS path, try to use default path:" + name);
